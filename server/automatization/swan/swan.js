...
const TASK_TYPES = require('app/server/automatization/swan/structures/taskTypes');
const { app } = require('electron');
const fs = require('fs');
const path = require('path');
const getRandomArrayElement = require('app/server/automatization/helpers/getRandomArrayElement');
const {
  hoverAndClick,
  scrollPage,
  wait,
} = require('../helpers/puppeteerHelpers.js');
const PageScroller = require('app/server/automatization/helpers/PageScroller');
...
async function openProfile(userId, dailyCombo, onlyDaily) {
  ...
  const response = await axios.get('http://local.adspower.net:58581/api/v1/browser/start', {
