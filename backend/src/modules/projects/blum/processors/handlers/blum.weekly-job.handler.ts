...
class BlumWeeklyJobHandler {
  ...
  async socialTasksDone(weeklyCard) {
    ...
    const progressElement = await weeklyCard.$(
      '.pages-tasks-progress-pill .label',
    );
  ...
  async processWeeklySocialTask(task) {
    ...
    const actionBtnText = await actionBtnEl.evaluate((el) =>
      el.innerText.trim(),
    );
  ...
  async getNotCompletedTasks() {
    const tasks = await this.blumFrame.$$(
      '.pages-tasks-subtasks-modal .nested-tasks > .pages-tasks-list-item-label',
    );
...
export default BlumWeeklyJobHandler;
