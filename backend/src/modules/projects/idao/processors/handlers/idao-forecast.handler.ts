...
import normalizeString from '../../../../../common/helpers/normalizeString.js';
import getTextInElement from '../../../../../common/helpers/puppeteer/getTextInElement.js';
import typeWithRandomDelay from '../../../../../common/helpers/puppeteer/typeWithRandomDelay.js';
...
export class IdaoForecastHandler {
  protected browser: Browser;
  protected page: Page;
  protected forecastOptions: IIdaoForecastOptions;
  protected walletService: RabbyService;
  private forecastPairs: TCurrencyPair[] = [
    'BTC/USD',
    'ETH/USD',
    'BNB/USD',
    'XRP/USD',
  ...
  private shortTermTimelines: TShortTermTimelines[] = [
    '1 hour',
    '4 hours',
    '12 hours',
  ];
  ...
  async getAvailableSlotsCount(term: string): Promise<number> {
    ...
    const slotText = await getTextInElement(
      this.page,
      availableSlotElements[0],
    );
  ...
  async pickTradingPair(pair: TCurrencyPair): Promise<void> {
    const selector =
      'xpath/.//div[contains(text(), "Select COIN")]/following-sibling::div//div[contains(@class, "cursor-pointer")]';
    ...
    if (
      normalizeString(await getTextInElement(this.page, openPairsModalBtn)) ===
      normalizeString(pair)
    ) {
      return;
    }
  ...
  async pickTimeline(timeline: TShortTermTimelines): Promise<void> {
    const selector =
      'xpath/.//div[contains(text(), "Choose horizon")]/following-sibling::div//div[contains(@class, "cursor-pointer")]';
    ...
    if (
      normalizeString(
        await getTextInElement(this.page, openTimelineModalBtn),
      ) === normalizeString(timeline)
    ) {
      return;
    }
  ...
  async pickOptionInModal(
    option: TCurrencyPair | TShortTermTimelines,
  ...
  async setForecastPrice() {
    ...
    const newPrice =
      currentPrice + sign * currentPrice * (targetPriceDeviation / 100);
  ...
  async makeForecast() {
    ...
    const walletPage = await this.walletService.waitForExtensionOpen(
      this.browser,
      30000,
    );
    await wait(1211, 3121);
    await this.walletService.signTransaction(walletPage, { maxGasFee: 0.05 });
  }
}
