import {wait} from './puppeteerHelpers';
...
class PageScroller {
  ...
  constructor({page, scrollableTag}: { page: Page | Frame; scrollableTag: string }) {
    this.page = page;
    this.scrollableTag = scrollableTag;
  }

  private async scroll({minDistance, maxDistance, minDelay, maxDelay, direction, targetPosition}: ScrollOptions) {
  ...
  async scrollToElementCenter(element: any, {
    minDistance = randomOptions.minDistance,
    maxDistance = randomOptions.maxDistance,
    minDelay = randomOptions.minDelay,
    maxDelay = randomOptions.maxDelay,
  ...
  private async getScrollTopPosition(tag: any): Promise<number> {
    return this.page.evaluate(tag => document.querySelector(tag)?.scrollTop || 0, tag);
  }

  private async getScrollHeight(tag: any): Promise<number> {
    return this.page.evaluate(tag => document.querySelector(tag)?.scrollHeight || 0, tag);
  }
}

function calculateRandomScrollDistance(minDistance: number, maxDistance: number): number {
